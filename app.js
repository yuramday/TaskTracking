...
function renderSidebar(){
  ...
  $('#workerList').innerHTML = state.workers.map((w,i) => `<button class="worker-item ${state.filter.workers.includes(w.name)?'selected':''}" data-filter-worker="${esc(w.name)}"><i class="worker-avatar" style="background:${colors[i%colors.length]}">${initials(w.name)}</i><span>${esc(w.name)}</span></button>`).join('');
